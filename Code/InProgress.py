...
class MyFloatLayout(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.button2 = Button(text='button2', size_hint=(None, None), size=(100, 50),
                             pos_hint={'x': 0.89, 'y': 0.90})
        self.button2.bind(on_press=self.on_button2_click) 
        self.button2.bind(on_press=self.start_timer) 
    ...
    def on_button2_click(self, instance):
        ...
        if not self.movement_in_progress:  # Check if no movement is in progress
            ...
            print("movement aan")
            # Move the pupil of the right eye smoothly to (50, 50)
            Clock.schedule_interval(self.move_pupil_smoothly2, 0.02)
    ...
    def move_pupil_smoothly2(self, dt):
        # Calculate the distance to move in each step
        step_x_right = (30 - self.right_eye.pupil_position[0]) / 5
        step_y_right = (30 - self.right_eye.pupil_position[1]) / 10

        step_x_left = (30 - self.left_eye.pupil_position[0]) / 5
        step_y_left = (30 - self.left_eye.pupil_position[1]) / 10
        ...
        # If reached the target position, stop the clock and reset movement state
        if abs(step_x_right) < 0.1 and abs(step_y_right) < 0.1:
            Clock.unschedule(self.move_pupil_smoothly2) 
            self.movement_in_progress = False
            print("movement uit")
